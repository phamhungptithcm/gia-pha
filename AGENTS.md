...
This project is indexed by GitNexus as **gia-pha** (6087 symbols, 9387 relationships, 294 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
| Resource | Use for |
|----------|---------|
| `gitnexus://repo/gia-pha/context` | Codebase overview, check index freshness |
| `gitnexus://repo/gia-pha/clusters` | All functional areas |
| `gitnexus://repo/gia-pha/processes` | All execution flows |
| `gitnexus://repo/gia-pha/process/{name}` | Step-by-step execution trace |
...
This project has a local CocoIndex repo inventory pipeline at
`scripts/cocoindex_repo_inventory.py`.

Run it from the repository root:

```bash
uv run --python 3.11 --with cocoindex cocoindex update scripts/cocoindex_repo_inventory.py -f
```

Outputs are local-only and ignored under `.cocoindex/` and `cocoindex-out/`.
