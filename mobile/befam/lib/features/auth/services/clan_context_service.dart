...
abstract interface class ClanContextService {
  Future<ClanContextSnapshot> loadContexts({required AuthSession session});
...
class FirebaseClanContextService implements ClanContextService {
  FirebaseClanContextService({FirebaseFunctions? functions, FirebaseAuth? auth})
  ...
  ClanContextOption _parseContext(Map<String, dynamic> map) {
    return ClanContextOption(
      ...
      primaryRole: (_stringOrNull(map['primaryRole']) ?? 'MEMBER')
          .toUpperCase(),
